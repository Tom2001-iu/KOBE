...
const ProductModal: React.FC<{
    ...
    const validate = () => {
        ...
        if (isNaN(stockNum) || stockNum < 0 || !Number.isInteger(stockNum)) newErrors.stock = 'Stock must be a non-negative integer.';
        
        if (imagePreviews.length === 0) newErrors.images = 'At least one product image is required.';
        else if (imagePreviews.length > 5) newErrors.images = "You can upload a maximum of 5 images.";
    ...
    }, [name, category, price, stock, imagePreviews, isOpen]);
    ...
    useEffect(() => {
        ...
        // Reset touched state and errors when modal opens or product changes
        setTouched({});
        setErrors({});
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setTouched({name: true, category: true, price: true, stock: true, images: true});
        if (!validate()) {
            return;
        }
        ...
        const submittedProduct: Product = {
            ...
            currency,
            status: parseInt(stock, 10) > 40 ? 'In Stock' : parseInt(stock, 10) > 0 ? 'Low Stock' : 'Out of Stock',
        };
            ...
            <div className="bg-brand-secondary rounded-lg shadow-xl p-6 md:p-8 w-full max-w-2xl animate-scale-in" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1">
                        <div>
                            <input type="text" placeholder="Product Name" value={name} onChange={e => setName(e.target.value)} onBlur={() => handleBlur('name')} className={`w-full bg-brand-dark rounded-lg py-2 px-4 focus:outline-none focus:ring-2 transition-colors ${touched.name && errors.name ? 'border-red-500 focus:ring-red-500' : 'border-gray-700 focus:ring-brand-accent'}`} />
                            {touched.name && errors.name && <p className="text-red-400 text-xs mt-1">{errors.name}</p>}
                        </div>
                        <div>
                            <input type="text" placeholder="Category" value={category} onChange={e => setCategory(e.target.value)} onBlur={() => handleBlur('category')} className={`w-full bg-brand-dark rounded-lg py-2 px-4 focus:outline-none focus:ring-2 transition-colors ${touched.category && errors.category ? 'border-red-500 focus:ring-red-500' : 'border-gray-700 focus:ring-brand-accent'}`} />
                            {touched.category && errors.category && <p className="text-red-400 text-xs mt-1">{errors.category}</p>}
                        </div>
                        <div className="mt-4">
                            <div className="flex space-x-2">
                                <input type="number" placeholder="Price" value={price} onChange={e => setPrice(e.target.value)} onBlur={() => handleBlur('price')} className={`w-full bg-brand-dark rounded-lg py-2 px-4 focus:outline-none focus:ring-2 transition-colors ${touched.price && errors.price ? 'border-red-500 focus:ring-red-500' : 'border-gray-700 focus:ring-brand-accent'}`} min="0.01" step="0.01" />
                        ...
                        <div className="mt-4">
                             <input type="number" placeholder="Stock" value={stock} onChange={e => setStock(e.target.value)} onBlur={() => handleBlur('stock')} className={`w-full bg-brand-dark rounded-lg py-2 px-4 focus:outline-none focus:ring-2 transition-colors ${touched.stock && errors.stock ? 'border-red-500 focus:ring-red-500' : 'border-gray-700 focus:ring-brand-accent'}`} min="0" step="1" />
                             {touched.stock && errors.stock && <p className="text-red-400 text-xs mt-1">{errors.stock}</p>}
                        </div>
                    </div>
                     <div className="pt-2">
                        <label className="block text-sm font-medium text-brand-text-secondary mb-2">Product Images</label>
                        <input type="file" multiple accept="image/*" onChange={handleImageChange} onBlur={() => handleBlur('images')} className="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-brand-accent file:text-white hover:file:bg-brand-accent-hover"/>
                        {touched.images && errors.images && <p className="text-red-400 text-xs mt-1">{errors.images}</p>}
                    ...
                    <div className="mt-6 flex justify-end space-x-4">
                        ...
                        <button type="submit" disabled={!isFormValid} className="bg-brand-accent hover:bg-brand-accent-hover text-white font-bold py-2 px-4 rounded-lg transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">{isEditMode ? 'Update Product' : 'Add Product'}</button>
                    </div>
                </form>
            </div>
        </div>
